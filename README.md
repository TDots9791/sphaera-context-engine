...
- пользователи совместимых AI-платформ.
...
| **Интеграция** | Основа — локальная работа через CLI и локальная база на устройстве; навыки поддерживаемых AI-платформ обращаются к одной карте. Локальный MCP-сервер (`mcp-stdio`) — опциональная поверхность, а не требование | Агент получает проверяемый контекст прямо в привычном рабочем цикле, без отдельного сервера и без обязательного MCP |
...
- **Одна карта для разных AI.** Совместимые AI-инструменты обращаются к одной локальной карте. При смене модели меняется объём и форма выдачи, а не заново создаётся память проекта.
